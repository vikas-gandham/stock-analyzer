...
if search_query:
    ...
    if df.empty or len(df) < 50:
        ...
        try:
            ...
            c3.metric("50 DMA", fmt_price(sma_50_val), delta=f"{ui_sign}{ui_dma_ext_pct:.1f}%", delta_color=metric_delta_color)
            c4.metric("Ideal Entry (Bounce)", fmt_price(ideal_entry))
            c5.metric(s_label, fmt_price(support_val), delta=s_delta, delta_color="normal")
            c6.metric("Auto Stop (Zone)", fmt_price(ideal_stop))
            c7.metric(r_label, fmt_price(resistance_val), delta=r_delta, delta_color="normal")
            c8.metric("52W High", fmt_price(week52_high))
            c9.metric("52W Low", fmt_price(week52_low))
